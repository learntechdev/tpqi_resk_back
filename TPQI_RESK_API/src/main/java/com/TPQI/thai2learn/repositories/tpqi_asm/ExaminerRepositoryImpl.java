...
public class ExaminerRepositoryImpl implements ExaminerRepository {
    ...
    @Override
    public Page<AssessmentInfoDTO> findAllExamRounds(String search, String qualification, String level, String tool, Pageable pageable) {
        String fromClause = """
            FROM exam_schedule es
            LEFT JOIN assessment_applicant aa ON es.tpqi_exam_no = aa.exam_schedule_id
            LEFT JOIN settings_tooltype st ON aa.asm_tool_type = st.id
    ...
    @Override
    public List<String> findAllDistinctOccLevelNames() {
        String sql = """
            SELECT DISTINCT es.occ_level_name 
            FROM exam_schedule es 
            WHERE es.occ_level_name IS NOT NULL AND es.occ_level_name != ''
    ...
    @Override
    public List<String> findDistinctOccLevelNamesByExamCodes(List<String> examCodes) {
        ...
        String sql = """
            SELECT DISTINCT es.occ_level_name 
            FROM exam_schedule es 
            WHERE es.tpqi_exam_no IN (:examCodes)
            AND es.occ_level_name IS NOT NULL AND es.occ_level_name != ''
            ORDER BY es.occ_level_name
        """;
        Query query = entityManager.createNativeQuery(sql, String.class);
        query.setParameter("examCodes", examCodes);
        return query.getResultList();
    }

    @Override
    public List<String> findDistinctAssessmentToolsByExamCodes(List<String> examCodes) {
        if (examCodes == null || examCodes.isEmpty()) {
            return new ArrayList<>();
        }
        String sql = """
            ...
            FROM assessment_applicant aa
            JOIN settings_tooltype st ON aa.asm_tool_type = st.id
            WHERE aa.exam_schedule_id IN (:examCodes)
